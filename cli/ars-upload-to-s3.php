 ...
 * ARS Download Repository Generator
 *
 * This CLI script serves to batch upload the Amazon S3 filesystem for Joomla's downloads
 *
 * @since  1.0
 */
class UploadPackages extends JApplicationCli
{
	/**
	...
	public function doExecute()
	{
		...
		$bucketPath = 'joomladownloads/joomla3/';

		$packageDir = __DIR__ . '/packages';

		$s3 = \Akeeba\ReleaseSystem\Admin\Helper\AmazonS3::getInstance();

		foreach (JFolder::files($packageDir) as $file)
		{
			$this->out('Processing ' . $file);

			$success = $s3->putObject($packageDir . '/' . $file, $bucketPath . $file);
			...
			else
			{
				$this->out(
					sprintf(
						'<error>An error occurred uploading the file: %s</error>',
						$s3->getError()
					)
				);
			}
...
try
{
	$app = JApplicationCli::getInstance('UploadPackages');
	set_exception_handler(['UploadPackages', 'handleException']);
	$app->execute();
}
catch (Exception $e)
{
