...
class ImportDownloads extends JApplicationCli
{
	...
	public function doExecute()
	{
		...
		require_once __DIR__ . '/ars-data/ars-categories.php';

		// Save our categories
		foreach ($categories as $category)
		{
			// Add additional data for the record
			$category = array_merge($category, ['created_by' => $user->id]);
			...
			try
			{
				...
				$this->out(sprintf('<info>Saved category "%s"</info>', $category['title']));
			}
			catch (Exception $e)
			{
				$this->out(sprintf('<error>Error saving category "%1$s": %2$s</error>', $category['title'], $e->getMessage()));
			}
		}
	}
